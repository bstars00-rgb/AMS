// Standalone port of the AMS room-scoring algorithm for the local agent.
// Adds Area (평수) and Smoke (흡연) — which Trip.com's mapping screen shows —
// and basic Chinese bed/grade/type handling.
...
// ---- bed normalization (EN + CN) ----
// Scans the whole string and collects EVERY bed type present (so "1 King bed"
// inside a name, or multi-option "SingleBed/King Bed", are all captured).
...
function cat(a, b) { if (!a && !b) return 0.5; if (!a || !b) return 0.5; return a === b ? 1 : 0; }
...
function areaScore(a, b) {
  ...
  return Math.max(0, 1 - diff / 10); // within ~10 sqm tolerance
}

// merchant / candidate: { name, bed, view, area, smoke }
export function scoreCandidate(merchant, cand, weights) {
  const mBeds = normalizeBeds(`${merchant.bed} ${merchant.name}`);
  const cBeds = normalizeBeds(`${cand.bed} ${cand.name}`);
  const bedE = bedEval(mBeds, cBeds);
  const parts = {
  ...
  const total = weights.name + weights.bed + weights.type + weights.grade + weights.view + weights.area + weights.smoke || 1;
  const score = Math.round(
    (weights.name * parts.name + weights.bed * parts.bed + weights.type * parts.type +
      weights.grade * parts.grade + weights.view * parts.view + weights.area * parts.area +
      weights.smoke * parts.smoke) / total
  );
